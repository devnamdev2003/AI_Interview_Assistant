...
EndInterview.addEventListener("click", function () {
  ...
  if (QA.length === 0) {
    let confirmation = confirm(
      "You have not provided any answers. Press OK to continue."
    );
    ...
    if (confirmation === true) {
      ...
      window.location.href = `./result.html?data=${encodeURIComponent(
        serializedQA
      )}&interviewData=${encodeURIComponent(serializedInterviewData)}`;
    }
  }
});
